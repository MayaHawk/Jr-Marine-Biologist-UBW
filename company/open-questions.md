...
**Purpose:** everything the team needs from Maya in order to stop guessing. Any agent may add a question. Only Maya's answers close one.
...
## PRIORITY 1 — Blocking almost everything
...
| N-16 | 🔴 **Where do the marine photographs come from?** Your own, licensed stock, or public-domain science imagery? **Usage rights must be confirmed before anything prints** — 122 cards is a commercial print run, not personal use. | Product, Maya | ⬜ |
| N-17 | ❓ **One word did not transcribe: "Monocles".** It is not one of the 24 topics. Did you mean a topic name, or something about the italics rule? | Product | ⬜ |
...
**Context:** the Sharks materials list has been rebuilt to the Marine Mammals standard from the verbatim Canva card content (`product-development/supply-lists/sharks.md`), and all ten cards audited against D-015 (`product-development/topics/sharks-card-checklist.md`). These are what stand between Sharks and a costed, orderable kit. **SH-2 and SH-3 block everything else.**

**Update 2026-09-19 — Maya confirmed:** *"Each age group has 5 activites. Each box is only 5 activites. You pick your age group."* That closes SH-1's numbering half and SH-21, and makes Sharks an **age-selected product**. The new work it creates is SH-20, SH-22, SH-23, SH-24, SH-26 and SH-27. **SH-25 — how "7 flash cards" squares with "5 activities per box" — remains unreconciled and no agent will guess at it.**
...
| **SH-20** | 🔴 **Print the age band on the front of every card?** Suggested: `Sharks · Ages 7–9 · Card 1 of 5`. **Why it matters:** you confirmed *"You pick your age group"* — so every sale carries a promise about which five cards are inside, and **with no age printed anywhere, the parent cannot verify they got the set they paid for.** The packer cannot verify it either, with two or three card sets per topic on the table. It also closes supply-list flag F-12, which records that **no age band appears on the younger tier's cards at all** and calls that safety-relevant, not cosmetic. **Marked 🔴 because it blocks typesetting** — decided after the ten fronts are set, all ten are re-set a second time, and the same choice then repeats across all 122 cards. **Interacts with SH-13:** topic + discipline + age band + number is four pieces of text on a front D-015 limits to two. | Product, Maya | ⬜ |
| ~~**SH-21**~~ | ~~How does a buyer choose their age tier?~~ ✅ **ANSWERED 2026-09-19 — *"You pick your age group."*** The tier is a **purchase choice**. Sharks is sold as an age-selected product. *(Whether the store actually presents that choice today is a separate question — SH-26.)* | Product, Sales, CX | ✅ |
| ~~**SH-26**~~ | ~~Does the store actually offer the age choice today?~~ ✅ **ANSWERED 2026-09-19 — Maya confirmed: *"Yes there is an age drop down."*** The storefront does present an age selector, so the tier **is** captured at the point of sale. *(What the dropdown's options are, and whether the chosen age reaches the packing table on the order confirmation, are now SH-28 and SH-29.)* | Sales, Ops, CX | ✅ |
| **SH-27** | 🟨 **What is packed if a buyer does NOT state an age group?** Less likely now the dropdown exists — but only if the field is **required**. If it can be left blank, or if any historic order predates the dropdown, the packer still needs a rule rather than a judgement call. *(See SH-28.)* | Ops, CX, Product | ⬜ |
| ~~**SH-28**~~ | ~~What are the age options in the dropdown?~~ ✅ **ANSWERED 2026-09-19 — *"4-6, 7-9 and 10+"*.** Three age groups are on sale. *(Whether the field is required is still unanswered; the consequences are SH-31, SH-32 and SH-33.)* | Product, Ops, Sales | ✅ |
| **SH-31** | **Band naming — the store says 7–9, `known-facts.md` says 7–10.** Both are your words, on different dates. The store wording is what customers have been choosing, so it is probably the right one to print — but it **appears on every card front**, so confirm it deliberately rather than by default. *(Assumption A-045 reads the unmarked Sharks tier as 7–10 and may need revisiting.)* | Product | ⬜ |
| **SH-32** | 🔴🔴 **AGE 4–6 IS ON SALE AND THE 4–6 SHARKS CARDS ARE NOT READY.** The dropdown offers 4–6; the Sharks 4–6 set is in the old design format, has no logo or card numbering, and is not in the ALL master. **Two questions, both urgent: (a) has anyone ordered a 4–6 kit, and what was shipped to them? (b) until the 4–6 cards are finished, should that option stay on sale?** Removing an option from a live store is a change to what is being sold — **your decision, not the team's.** | Product, Ops, Sales, Maya | ⬜ |
| ~~**SH-33**~~ | ~~Does every topic sell in three age groups?~~ ✅ **ANSWERED 2026-09-19 — *"Every topic you can pick one of the three age groups."*** So the design job is **24 topics × 3 age groups × 5 activity cards = ~360 activity card designs**, not 122. 🔴 **This needs to reach the bulk print quote before it is requested** — the Christmas order-by is ~Oct 15 and the quantity being quoted would otherwise be wrong. *(The species flash-card count is separate and still unreconciled — SH-25.)* | Product, Finance, Ops | ✅ |
| **SH-29** | 🔴 **Does the chosen age appear on the order confirmation the packer works from?** A dropdown that captures the age but does not carry it onto the order leaves the packing table exactly as blind as no dropdown at all. **Check one real order email.** *(Blocked on my side — Gmail authorization has dropped.)* | Ops, CX | ⬜ |
| **SH-30** | **Does the age dropdown exist on BOTH storefronts?** Maya confirmed a dropdown; which store she was looking at is not recorded. Payments are taken on **both** GoDaddy and Under Blue Waters *(CONFIRMED)*, so an age selector on one and not the other still leaves untiered orders arriving. | Sales, Ops | ⬜ |
| **SH-22** | 🔴 **Is each age group a separate product listing / SKU, or one listing with an age option?** This decides the store structure, the SKU count and how the tier arrives at the packing table. *(Related to U-30, which asked the same thing before the one-tier-per-kit answer existed.)* | Product, Sales, Ops | ⬜ |
| **SH-23** | 🔴 **On a bulk order of 50–100 kits to a school or camp, is the age group chosen ONCE for the whole order, or per kit?** A single classroom is usually one age; a camp or co-op usually is not. **This bears directly on the 2,000-kit goal:** one age group for the order means one pick list and a fast pack; per-kit choice means collecting 50–100 individual ages and picking each box separately. **Needed before the first bulk quote goes out**, because it changes both the price and the promise. | Sales, Ops, Product | ⬜ |
| **SH-24** | **Does Age 4–6 become a THIRD selectable age group, or a digital/bonus set?** A complete Age 4–6 Sharks set exists in Canva but sits outside the ALL master *(N-24, SH-18)*. If it becomes selectable, the buyer picks from three groups and Sharks is three variants, not two — one more card set to design, print, stock and pick. | Product, Maya | ⬜ |
| **SH-25** | 🟨 **PARTLY ANSWERED 2026-09-19 — *"7 flash cards is including the identification sheet attached with one of the shark attivities."*** So the 7 counts **printed items in the kit**, not seven species cards. **Still one short:** 5 activity cards + 1 identification sheet = **6**. ❓ **What is the 7th?** The completion certificate is the obvious candidate — you said you were adding certifications to the set — but that is a question, not an assumption, and nothing has been costed on it. | Product, Finance, Ops | ⬜ |
...
| N-22 | ~~Where are the collectible species flash cards?~~ **ANSWERED: in `Making Sense Of Sharks Age 7-10` — 16 species with lengths.** ✅ | Product | ✅ |
| N-27 | 🔴 **How many species cards go in a Sharks kit — 7, 10, or 16?** Three different numbers are now on record: known-facts says 7, the Art card lists 10, the set holds 16. At 16 the printed card cost per kit more than doubles. | Product, Finance | ⬜ |
| N-28 | 🔴 **Check the dwarf lanternshark length on the card (SV-13).** It should be ~8 inches, not 4–5 feet. The Math activity depends on correct lengths. | Maya | ⬜ |
| ~~N-22 old~~ | ~~Where are the COLLECTIBLE SPECIES flash cards?~~ Two activity cards depend on them — the Art card says *"use the shark flashcards to choose the tooth"* and the Math card says *"place your shark flashcards along the yarn, smallest to largest"*. The file named "Shark flash cards" turned out to be activity cards in print layout, not species cards. **Do the species cards exist?** | Product | ⬜ |
...
| U-38 | 🔴 **Get a bulk print quote for flash cards.** ~120–130 unique designs: 7 for Sharks, 5 for every other kit. $0.71/card was a TRIAL price — your production cost is unknown, and cards are the largest component identified so far. Nothing about kit margin is settled until this exists. | Maya | ⬜ |
...
| U-26 | **When you REPLY, which address does it go out from?** A customer who wrote to Jr. Marine Biologist and gets a reply from Under Blue Waters may be confused. | Customer Experience | ⬜ |
| U-27 | **Does your site list JrMarinebiologist@gmail.com, or a different address?** A web search turned up "JrMarineBiology@gmail.com" — probably just a bad search result (that source has been wrong twice), but worth a 30-second check that no customer mail is going to an address you never read. | Customer Experience | ⬜ |
...
## PRIORITY 2 — Needed within the first month
...
| # | Question | Status |
|---|---|---|
| 16 | Is QuickBooks already set up? Which version, and is a chart of accounts in place? | ⬜ |
| 17 | Do you have a bookkeeper or CPA today? | ⬜ |
| 18 | What is the legal entity (sole proprietor, LLC, S-corp) and which state? | ⬜ |
| 19 | Monthly fixed costs today (software, storage, insurance, subscriptions)? | ⬜ |
| 20 | Are you currently spending anything on advertising? | ⬜ |
| 21 | How much cash can be committed to inventory before revenue arrives? | ⬜ |
| 22 | What gross margin would you consider healthy? | ⬜ |

### Operations
| # | Question | Status |
|---|---|---|
| 23 | Who assembles the kits today — you, family, or a contractor? | ⬜ |
| 24 | Where is inventory stored, and how much space is there? | ⬜ |
| 25 | Which suppliers do you already use? | ⬜ |
| 26 | What carrier and service do you ship with, and at what cost? | ⬜ |
| 27 | On what day of the month do subscription kits ship? | ⬜ |
| 28 | What is the current policy for damaged or lost kits? | ⬜ |

### Sales & Customers
| # | Question | Status |
|---|---|---|
| 29 | Where have your customers come from so far? | ⬜ |
| 30 | Any existing school, camp, co-op, or dive shop relationships? | ⬜ |
| 31 | Do you have a current subscription cancellation rate? | ⬜ |
| 32 | What is your refund/return policy today? | ⬜ |
| 33 | What are the most common questions customers ask you? | ⬜ |

### Brand
| # | Question | Status |
|---|---|---|
| 34 | Do you have a logo, brand colors (hex codes), and fonts? | ⬜ |
| 35 | Which brand is customer-facing on the box — Junior Marine Biologist, Under Blue Waters, or both? | ⬜ |
| 36 | Exact wording you want used for your credentials in public copy? | ⬜ |
| 37 | Is the business name singular ("Junior Marine Biologist") or plural ("Junior Marine Biologists")? | ⬜ |

---

## PRIORITY 3 — Needed before scaling

| # | Question | Status |
|---|---|---|
| 38 | Do you want to sell wholesale, and at what minimum order? | ⬜ |
| 39 | Geographic focus — South Florida, US-wide, or international shipping? | ⬜ |
| 40 | Do you want to hire help in the next 12 months? | ⬜ |
