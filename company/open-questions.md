...
| **SH-33** | 🔴 **Does EVERY topic sell in three age groups, or only Sharks?** `known-facts.md` records **122 unique card designs** on the assumption of one set per topic. Sharks alone has two activity sets in the master plus a third outside it. **If all 24 topics are sold in three age groups, the true design count is a multiple of 122** — which changes the print run, the print quote you are chasing for Christmas, the card cost per kit, and how long finishing the catalog takes. **No agent has extrapolated this and none will.** One sentence from you sizes the whole remaining design job. | Product, Finance, Ops, Maya | ⬜ |
