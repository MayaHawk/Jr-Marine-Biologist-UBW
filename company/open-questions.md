...
| **SH-26** | 🔴 **Does the store ACTUALLY offer the age choice today?** Maya has confirmed the buyer picks their age group — but **neither the GoDaddy store nor Under Blue Waters has been verified as presenting an age selector.** If it does not, **orders are arriving with no tier recorded and nobody knows which five cards to pack.** ⚠️ **This affects orders already in hand, not just future ones** *(see N-1, U-1 — kits are waiting to ship right now)*. A 60-second look at your own product page answers it. | Sales, Ops, CX, Maya | ⬜ |
| **SH-27** | 🔴 **What is packed if a buyer does NOT state an age group?** There is no default on record. Options: pack a chosen default, email and ask (delays the order), or make the field required at checkout. **Whoever packs needs a rule, not a judgement call.** | Ops, CX, Product | ⬜ |
